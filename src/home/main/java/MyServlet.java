...
import home.main.java.html.builders.DocumentBuilder;
import home.main.java.html.builders.DocumentDirector;
import home.main.java.html.builders.IndexBuilder;
...
public class MyServlet extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		if(request.getAttribute("load")!=null){
			request.getRequestDispatcher(request.getRequestURI().substring(1)).include(request,response);
			response.flushBuffer();
			return;
		}

		DocumentBuilder documentBuilder=new IndexBuilder();
		LOG.info(request.getRequestURI());
		response.getOutputStream().print(
				DocumentDirector.create(documentBuilder).getHtml());
		return;
